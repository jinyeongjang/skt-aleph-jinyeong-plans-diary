...
- **과제 6 & 과제 7 세부 명세 및 증빙 문서**:
  - 📋 [공식 과제 제출 요건 전수 점검표 (`docs/SUBMISSION_SPECIFICATION.md`)](docs/SUBMISSION_SPECIFICATION.md)
  - 📊 [5일 연속 관찰 기록 및 수기 검산 명세서 (`docs/OBSERVATION_5DAYS_SPEC.md`)](docs/OBSERVATION_5DAYS_SPEC.md)
  - 🔐 [인증 구현 설명서 6대 항목 가이드 (`docs/AUTH_IMPLEMENTATION_GUIDE.md`)](docs/AUTH_IMPLEMENTATION_GUIDE.md)
  - 📑 [과제 통과 기준(T06/T07) 상세 매트릭스 (`CRITERIA.md`)](CRITERIA.md)
...
② 세 단계 안에 무엇을 하나요:
   1) 첫 화면의 [User A(장진영) 빠른 로그인]을 눌러 인증을 통과한 뒤 [5일 관찰 대시보드]에서 1~5일차 기록과 3일차 진입 전(2026-09-18 22:30 KST) 계획 규칙 변경(20% 버퍼) 및 수기 검산 일치표를 확인합니다.
...
1. **회원가입 흐름 (`authService.register`)**:
   - 위치: [`src/services/authService.ts`](src/services/authService.ts#L77-L106)
...
2. **로그인 흐름 (`authService.login`)**:
   - 위치: [`src/services/authService.ts`](src/services/authService.ts#L108-L138)
...
3. **로그아웃 흐름 (`authService.logout`)**:
   - 위치: [`src/services/authService.ts`](src/services/authService.ts#L140-L155)
...
4. **자료 조회 및 격리 흐름 (`pdsService.getPlans`, `pdsService.getTodos`, `pdsService.getTodoById`)**:
   - 위치: [`src/services/pdsService.ts`](src/services/pdsService.ts#L38-L52)
...
> 📄 인증 구현 설명서 6대 항목의 전문 및 심층 분석은 [`docs/AUTH_IMPLEMENTATION_GUIDE.md`](docs/AUTH_IMPLEMENTATION_GUIDE.md)에서 확인하실 수 있습니다.
...
본 프로젝트는 실제 교육 과정(SKT ALEPH 1기) 실습을 바탕으로 작성된 **5일간(2026-09-17 ~ 2026-09-21 KST)의 실제 관찰 기록**을 포함합니다.

> 📄 5일 연속 관찰 원본 일지, 5대 예외 처리 규정 및 수기 검산 상세 문서는 [`docs/OBSERVATION_5DAYS_SPEC.md`](docs/OBSERVATION_5DAYS_SPEC.md)에서 확인하실 수 있습니다.

### 1) 고정 관찰 기준 (T07-C04 ~ T07-C06)

- **관찰 질문 (한 문장 고정, T07-C04)**: _"계획 대비 실제 소요 시간의 편차를 줄이고 예측 정확도를 80% 이상으로 유지할 수 있는가?"_
- **관찰 지표 (한 개 고정, T07-C05)**: **시간 오차 편차 (Time Variance = 실제 소요 시간 - 예상 소요 시간)**
- **측정 단위 (고정, T07-C06)**: **시간(분)**

### 2) 5대 데이터 처리 및 예외 규정 (T07-C23 ~ T07-C27)

1. **결측치 처리 (T07-C23)**: 소요 시간 누락 시 기본 추정치(0분) 부여 및 비정상 결측 플래그 표기.
2. **중복치 처리 (T07-C24)**: 동일 작업에 대한 중복 실행 기록은 멱등키(`idempotency_key`) 기준 최초 1건만 인정.
3. **이상치 처리 (T07-C25)**: 단일 작업 480분(8시간) 초과 시 이상치로 분리 집계 및 사유 명시.
4. **반올림 규칙 (T07-C26)**: 모든 비율(%) 및 평균 연산은 소수점 둘째 자리에서 반올림하여 소수점 첫째 자리까지 표기.
5. **주 시작 요일 (T07-C27)**: 대한민국 표준 ISO 8601 기준 **월요일(Monday, Day 1)**로 고정 정의.

### 3) 3일차 계획 규칙 변경 명세 (T07-C09 ~ T07-C15)

- **규칙 변경 시점 (T07-C09, C10)**: **2026-09-18T22:30:00+09:00 KST** (2일차 완료 후, 3일차 시작 전)
- **변경 근거 (T07-C12)**: 1일차(+10분) 및 2일차(+15분) 기록을 근거로 지정
- **변경 이유 (T07-C11)**: 1·2일차 실습 시 네트워크 패킷 환경 설정 및 트러블슈팅 지연이 반복되어, 3일차부터 **예상 시간에 20% 안전 버퍼를 추가 반영**하기로 결정.
- **변경 전 규칙 (T07-C22)**: 단순 작업 난이도 기준 예상 시간 산정 (별도 지연 안전 버퍼 없음, 버퍼 0%)
- **변경 후 규칙**: 네트워크 환경 설정 및 트러블슈팅을 감안하여 기본 예상 시간에 20% 안전 버퍼 추가 반영 (예: 기본 90분 -> 110분)

### 4) 5일 관찰 기록 타임라인 및 수기 검산 일치표 (T07-C07, T07-C132)

|     일차     |   관찰 일자 (KST)    | 관찰 실습 과제 내용                                             |         예상시간          | 실제시간  | 소요오차  |   완료상태   |      막힘여부      |        적용 규칙        |
| :----------: | :------------------: | :-------------------------------------------------------------- | :-----------------------: | :-------: | :-------: | :----------: | :----------------: | :---------------------: |
|  **Day 1**   |      2026-09-17      | Wireshark 패킷 캡처 및 TCP 3-Way Handshake 분석                 |           90분            |   100분   | **+10분** |  완료 (1개)  |        없음        |    변경 전 (버퍼 0%)    |
|  **Day 2**   |      2026-09-18      | DNS 계층 구조 및 캐싱 메커니즘 쿼리 분석                        |           60분            |   75분    | **+15분** |  완료 (1개)  |        없음        |    변경 전 (버퍼 0%)    |
| **규칙변경** | **2026-09-18 22:30** | **[계획 규칙 변경] 예상 시간에 20% 디버깅 안전 버퍼 추가 반영** |             -             |     -     |     -     |      -       |         -          |   **규칙 전환 시점**    |
|  **Day 3**   |      2026-09-19      | L2/L3 스위치 기반 VLAN 분할 및 IP 라우팅 망구성                 | 90분 (+20분 버퍼) = 110분 |   115분   | **+5분**  |  완료 (1개)  | 발생 (OSPF 디버깅) |   변경 후 (버퍼 20%)    |
|  **Day 4**   |      2026-09-20      | 리눅스(Ubuntu) iptables 및 ufw 방화벽 보안 정책 수립            | 75분 (+15분 버퍼) = 90분  |   85분    | **-5분**  |  완료 (1개)  |        없음        |   변경 후 (버퍼 20%)    |
|  **Day 5**   |      2026-09-21      | 제로 트러스트(Zero Trust) 엔드포인트 접근 제어 및 네트워크 감사 | 75분 (+15분 버퍼) = 90분  |   90분    |  **0분**  |  완료 (1개)  |        없음        |   변경 후 (버퍼 20%)    |
|   **합계**   |     **5일 연속**     | **총 5개 실습 과제 전수 수행 완료**                             |         **440분**         | **465분** | **+25분** | **5개 완료** |    **1회 발생**    | **수기 검산 100% 일치** |

- **수기 계산 검산 검증 (T07-C132)**:
  - 총 예상 시간 합계: $90 + 60 + 110 + 90 + 90 = 440\text{분}$
  - 총 실제 시간 합계: $100 + 75 + 115 + 85 + 90 = 465\text{분}$
  - 총 시간 오차: $465 - 440 = +25\text{분}$
  - 규칙 변경 전 (1~2일차) 일평균 오차: $(10 + 15) / 2 = +12.5\text{분}$
  - 규칙 변경 후 (3~5일차) 일평균 오차: $(5 + (-5) + 0) / 3 = 0.0\text{분}$
...
> 전체 세부 통과 기준 및 소스 코드 위치 매핑은 [`CRITERIA.md`](CRITERIA.md) 및 [`docs/SUBMISSION_SPECIFICATION.md`](docs/SUBMISSION_SPECIFICATION.md)에서 확인할 수 있습니다.
